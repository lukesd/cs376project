...
Musical quality is an inherently subjective judgement, and thus difficult to objectively assess.  However by soliciting judgements from a large number of people we can hope to average out personal preferences and converge on some measure of quality.  We created a web site which asked visitors to compare recordings of two duets and indicate which was better.  We advertised for evaluators by posting on Facebook and various email lists, and within 48 hours we gathered 474 evaluations, coming from 114 different IP addresses. By using the Elo rating system, which computes the relative scores of a group of ordered pairs, we were able to rank duets in terms of musical quality.
...
We recorded all note data during the duet performances, and calculated three metrics to quantify the musical parameter of note density. The \emph{percent of notes played} tells us how many total notes were played. The \emph{percent of time with no notes} is how often neither player was playing, \emph{i.e.} how much space the players left. And the \emph{percent of notes with only one player} is the proportion of time when a note was played that only one player was playing, which indicates how much the players took turns. Each of these metric has an obvious trend when we plot from the duets from the Control group to those of the Experiment group (see figure~\ref{note-stats}.) The first increased, while the second two decrease. A significant difference between these results appears when we compare the second duet of the control group with that of the experimental group. The control group played fewer notes ($p = 0.048$) and had higher proportion of single notes, \emph{i.e.} they exhibited more turn taking ($p = 0.035$). In some sense this suggests that the control group played better music during the second duet. 
...
Our goal was to better understand musical interactions for novices by dividing the interface into instrument and interactions, and then investigating the effect of adding structure to the interaction.  Our crowd-sourced evaluation of the musical results showed that adding structure clearly leads to better music.  However it is less clear whether adding structure led to a better experience for the user.  
...
The first is that the second duet was better because the previous structured interaction increased the users' confidence in performing duets with the instrument. From the interviews a few people (3 out of 8) reported that they liked the structural constraints because, like a game, they provided goals. However, none reported that they thought of the constraints during the second duet. Thus, further studies might: 

\begin{itemize}
\item determine whether having structure actually teaches (consciously or subconsciously) musical principles to novices users.
\item treat the first duets explicitly as \textit{training} (instead of a performance), and investigate how this training affects musical quality and user experience in subsequent unstructured performances.  This might lead to a subtly different experiment with a significantly different focus.
\end{itemize}

The second interpretation of the increase in enjoyment from structured to unstructured interaction, is that the structured interactions were not as enjoyable as unstructured.  A number of participants (4 out of 8) reported that it was difficult to quickly understand all the visual feedback being provided during structured interactions.  Though we attempted to design a simple instrument with simple interaction constraints, it seems the net effect was to create a complicated interaction.

Despite the differences between the interaction types, almost all participants reported enjoying the experience, and being pleased with being able to easily make good-sounding music.
...
The authors would like to acknowledge Professor Scott Klemmer, Chinmay Kulkarni, our experiment participants for their time and input, and the crowd of online evaluators for their opinions.
