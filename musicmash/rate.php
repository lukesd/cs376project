...
if ($_GET['winner'] && $_GET['loser']) {
	...
	// Get the winner
	$result = mysql_query("SELECT * FROM musics WHERE `music_id` = ".$_GET['winner']." ");
	$winner = mysql_fetch_object($result);


	// Get the loser
	$result = mysql_query("SELECT * FROM musics WHERE `music_id` = ".$_GET['loser']." ");
	...
	mysql_query("UPDATE musics SET `score` = ".$winner_new_score.", `wins` = `wins`+1 WHERE `music_id` = ".$_GET['winner']);
	...
	mysql_query("UPDATE musics SET `score` = ".$loser_new_score.", `losses` = `losses`+1  WHERE `music_id` = ".$_GET['loser']);
